...
class BSNL_Parser(object):
    ...
    def process(self, method, inputs):
        ...
        elif method == "L":  #By Last Name
            self.search_by_last_name(inputs[0])
    ...
    def search_by_last_name(self, input):
        ...
        city = input["City"]
        url_by_lastname = 'http://dq.wdc.bsnl.co.in/bsnl-web/residentialSearch.seam'
        url_content = 'http://dq.wdc.bsnl.co.in/bsnl-web/resSrchDtls.seam'
        self.browser.open(url=self.main_url)
        self.browser.open(url=url_by_lastname)
        f1 = self.browser.get_form(id="residential")
        #print(f1.fields)
        f1.fields['residential:firstField'].value = l_name
        f1.fields['residential:city'].value = city
        self.browser.submit_form(f1,submit="residential:button1")
        self.browser.open(url=url_content)
        body_table = self.browser.find(id="contentSrchResult").find('table').find('tbody')
        #body_table = body.find('table')
        #print(body_table)
...
if __name__ == '__main__':
    print("Started")
    ms = BSNL_Parser()
    ms.process("L", [{"LastName": "Sable", "City": "PUNE"}])
