    ...
    \begin{column}[T]{0.5\linewidth}
      \begin{block}{Origins}
        \begin{itemize}
          ...
          \item \textbf{2007, 2012}, Further Nobel awards for Mechanism Design (Hurwicz, Maskin, Myerson) and Matching Theory (Roth, Shapley)
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}[T]{0.48\linewidth}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Foundational Concepts}
\begin{center}
...
\begin{columns}
    ...
    \begin{column}[T]{0.5\linewidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Case 1: Opponent chooses A}: So look at 1st numbers (as they are for YOU) in column of `A'
        \begin{itemize}
          \item Option 1 gives you 3
          \item Option 2 gives you 4
          \item $\Rightarrow$ Option 2 is better. Underline `Option 2'.
        \end{itemize}
        
        \item \textbf{Case 2: Opponent chooses B}: So look at 1st numbers (as they are for YOU) in column of `B'
        \begin{itemize}
          \item Option 1 gives you 1
          \item Option 2 gives you 2
          \item $\Rightarrow$ Option 2 is again better. Underline `Option 2' again.
        \end{itemize}
        
        \item \textbf{Key Insight:}  
        No matter what opponent does (A or B), Option 2 always gives you a higher payoff. Two  underlines.
        
        \item \textbf{Conclusion:} Option 2 is a dominant strategy. 
        \item Exercise: do same procedure for your Opponent and find Dominant Strategy between 'A' and 'B'. Here we have to look at 2nd numbers in rows, ie. `Option 1' row and then `Option 2' row. Find which `A' or `B' is dominant. Assume that its `B'. Then their combination ie `Option 2' and `B' it cell $(4,2)$ is Dominant Strategy Equilibrium.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{itemize}
  \item If Player 2 plays Left: compare 5 vs 4 $\Rightarrow$ \underline{5 is higher} ie 'Up'
  \item If Player 2 plays Right: compare 1 vs 2 $\Rightarrow$ \underline{2 is higher} ie 'Down'
  \item As they both don't coincide, so there is No Dominant Strategy for Player 1
\end{itemize}
...
\begin{itemize}
  \item If Player 1 plays Up: compare 5 vs 4 $\Rightarrow$ \underline{5 is higher} ie. 'Left'
  \item If Player 1 plays Down: compare 1 vs 2 $\Rightarrow$ \underline{2 is higher} ie. 'Right'
  \item As they both don't coincide, so there is No Dominant Strategy for Player 2
\end{itemize}
...
No Dominant Strategy for either player. We will not go by such strictness and relax a bit.
Lets see if 'Player 1' choice is optimal for 'Player 2' and vice versa. That's Nash Equilibrium.
...
\textbf{Key Idea:}
\begin{itemize}
  \item Here Dominant Strategy Equilibrium does NOT exists.
...
\section{Foundational Strategies}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Foundational Strategies}
\begin{center}
{\Large Foundational Strategies}
\end{center}
\end{frame}
...
% \begin{columns}
    % \begin{column}[T]{0.58\linewidth}
      % \textbf{The Scenario:}
      % \begin{itemize}
        % \item Both locked in war over same 50 visitors who cross street every morning
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Zero-Sum Game: Ramesh-Suresh Customer War}

\textbf{The Scenario:}
\begin{itemize}
  \item Ramesh and Suresh compete for the same 50 customers every morning
  \item The customer pool is fixed
  \item If one gains customers, the other loses customers
  \item Their actions (discounts, louder music, signboards) only \textbf{shift customers}, not create new ones
\end{itemize}

\vspace{6pt}

\textbf{Customer Distribution Matrix:}
...
\textbf{Why is this Zero-Sum?}
\begin{itemize}
  \item In every outcome, total customers remain 50:
  \[
  25+25=50,\quad 15+35=50,\quad 35+15=50
  \]
  \item One player's gain is exactly the other's loss
\end{itemize}
...
\begin{itemize}
  \item If Suresh plays Normal:
  \begin{itemize}
    \item Ramesh gets 25 with Normal
    \item Ramesh gets 35 with Aggressive
    \item $\Rightarrow$ \underline{Aggressive} is better
  \end{itemize}

  \item If Suresh plays Aggressive:
  \begin{itemize}
    \item Ramesh gets 15 with Normal
    \item Ramesh gets 25 with Aggressive
    \item $\Rightarrow$ \underline{Aggressive} is better
  \end{itemize}
\end{itemize}
...
\textbf{Similarly for Suresh:}

\begin{itemize}
  \item Whether Ramesh plays Normal or Aggressive, Suresh also gets more customers by choosing \underline{Aggressive}
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item \underline{Aggressive} is the dominant strategy for both players
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nash Equilibrium and Strategic Insight}

\textbf{Since both players have the dominant strategy \underline{Aggressive}, the Nash equilibrium is:}

\[
\textbf{(Aggressive,\ Aggressive)}
\]

\vspace{6pt}

\textbf{Resulting Payoff Matrix:}
...
\begin{itemize}
  \item If one player stays Normal while the other becomes Aggressive, the aggressive player captures more customers
  \item So both are pushed toward Aggressive behavior
  \item But when both become Aggressive, each ends up with only 25 customers again
\end{itemize}
...
\textbf{Meaning:}
\begin{itemize}
  \item Both compete harder, but neither gains extra customers
  \item This is the \textbf{competitive trap} in zero-sum games
\end{itemize}
...
\begin{columns}
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
        \item \textbf{Introduction}: Positive-sum games allow the total gains to grow, ``growing the pie'' rather than fighting over fixed pool. Trade, collaboration, and innovation are classic examples where mutual gain is possible.
    ...
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
        ...
        \end{itemize}\item \textbf{Examples}: Pune's tech ecosystem where new firms benefit incumbents (including auto sector, via CAD-CAM); open-source software; international trade agreements; ride-sharing platforms benefiting both drivers and riders.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Positive-Sum Game: Ramesh-Suresh Grow Together}

\textbf{The Scenario:}
\begin{itemize}
  \item Both improve coffee quality, seating, and overall experience
  \item The street becomes a destination, not just a pass-through
  \item New customers arrive: office workers detour, weekend crowds grow
  \item Suppliers improve, partnerships (e.g., bakery) emerge
  \item Each player's improvement increases demand for both
\end{itemize}
...
\begin{tabular}{|c|c|c|}
  ...
  \textbf{Low Quality} & (25, 25) & (30, 40) \\
  \hline
  \textbf{High Quality} & (40, 30) & (55, 55) \\
  \hline
\end{tabular}
\end{center}
...
\textbf{Why is this Positive-Sum?}
\begin{itemize}
  \item Total customers increase when quality improves:
  \[
  25+25=50,\quad 30+40=70,\quad 55+55=110
  \]
  \item Both players can gain at the same time
\end{itemize}
...
\begin{itemize}
  \item If Suresh chooses Low Quality:
  \begin{itemize}
    \item Ramesh gets 25 (Low), 40 (High)
    \item $\Rightarrow$ \underline{High Quality} is better
  \end{itemize}
  
  \item If Suresh chooses High Quality:
  \begin{itemize}
    \item Ramesh gets 30 (Low), 55 (High)
    \item $\Rightarrow$ \underline{High Quality} is better
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Whether Ramesh chooses Low or High, Suresh gets more customers with \underline{High Quality}
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item \underline{High Quality} is the dominant strategy for both players
\end{itemize}
...
\textbf{Since both players have dominant strategy \underline{High Quality}, the Nash equilibrium is:}

\[
\textbf{(High\ Quality,\ High\ Quality)}
\]
...
\begin{tabular}{|c|c|c|}
  ...
  \hline
  High Quality & (40, 30) & \textbf{(55, 55)} \\
  \hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item When both invest in quality, total customers increase significantly
  \item Each player's action creates value for the other
  \item Competition here leads to \textbf{market expansion}, not just redistribution
\end{itemize}
...
\textbf{Meaning:}
\begin{itemize}
  \item Unlike zero-sum games, both players win together
  \item The real goal is not to steal customers, but to \textbf{grow the market}
\end{itemize}
...
% \begin{columns}
    % \begin{column}[T]{0.6\linewidth}
      ...
      % \begin{tabular}{|c|c|c|}
        ...
        % \hline
        % \textbf{Cooperate} & (50k, 50k) & (20k, 70k) \\
        % \hline
...
\textbf{The Scenario:}
\begin{itemize}
  \item Both shops charge fair prices and earn good profits
  \item Each can choose:
  \begin{itemize}
    \item \textit{Cooperate} (maintain fair pricing)
    \item \textit{Defect} (cut prices aggressively)
  \end{itemize}
  
  \item If both cooperate: stable profits (50k, 50k)
  \item If one defects: defector gains customers, other loses
  \item If both defect: price war → low profits for both
\end{itemize}

\vspace{6pt}

\textbf{Payoff Matrix (Monthly Profit):}
...
\textbf{Key Tension:}
\begin{itemize}
  \item Cooperation gives higher joint profit
  \item But fear of being undercut pushes players toward defection
\end{itemize}
...
\begin{itemize}
  \item If Suresh cooperates:
  \begin{itemize}
    \item Cooperate = 50k, Defect = 70k
    \item $\Rightarrow$ \underline{Defect} is better
  \end{itemize}
  
  \item If Suresh defects:
  \begin{itemize}
    \item Cooperate = 20k, Defect = 30k
    \item $\Rightarrow$ \underline{Defect} is better
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Whether Ramesh cooperates or defects, Suresh earns more by choosing \underline{Defect}
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item \underline{Defect} is the dominant strategy for both players
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nash Equilibrium and the Paradox}

\textbf{Since both players have dominant strategy \underline{Defect}, the Nash equilibrium is:}
...
\begin{tabular}{|c|c|c|}
  ...
  & Cooperate & Defect \\
  \hline
  Cooperate & (50k, 50k) & (20k, 70k) \\
  \hline
  Defect & (70k, 20k) & \textbf{(30k, 30k)} \\
  \hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item Both players end up with (30k, 30k)
  \item But (50k, 50k) is clearly better for both
\end{itemize}
...
\begin{itemize}
  \item Fear of being exploited:
  \begin{itemize}
    \item “If I cooperate and the other defects, I get only 20k”
  \end{itemize}
  \item So both choose to defect to protect themselves
\end{itemize}
...
\textbf{Final Insight:}
\begin{itemize}
  \item Individual rationality leads to a worse collective outcome
  \item This is the core of the \textbf{Prisoner’s Dilemma}
\end{itemize}
...
\textbf{The Scenario:}
\begin{itemize}
  \item Both shops consider staying open 24/7 to outlast the other
  \item High costs: electricity, staff, operations
  \item Like two trucks heading toward each other
  \item If one backs down, the other captures more customers
  \item If neither backs down, both suffer heavy losses
\end{itemize}

\vspace{6pt}

\textbf{Payoff Matrix (Monthly Profit/Loss):}
...
\textbf{Key Tension:}
\begin{itemize}
  \item You want to stay aggressive (24/7) if the other backs down
  \item But if both stay aggressive → disaster (-20k, -20k)
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Best Responses (No Dominant Strategy)}

\textbf{For Ramesh:}

\begin{itemize}
  \item If Suresh chooses Normal:
  \begin{itemize}
    \item Normal = 40k, 24/7 = 55k
    \item $\Rightarrow$ \underline{24/7} is better
  \end{itemize}
  
  \item If Suresh chooses 24/7:
  \begin{itemize}
    \item Normal = 10k, 24/7 = -20k
    \item $\Rightarrow$ \underline{Normal} is better
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item If Ramesh chooses Normal → \underline{24/7} is better
  \item If Ramesh chooses 24/7 → \underline{Normal} is better
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item No dominant strategy exists
  \item Best action depends on what the other player does
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nash Equilibria and Strategic Insight}

\textbf{Marking best responses in the matrix:}
...
\begin{tabular}{|c|c|c|}
  ...
  & Normal & 24/7 \\
  \hline
  Normal & (40k, 40k) & (\underline{10k}, \underline{55k}) \\
  \hline
  24/7 & (\underline{55k}, \underline{10k}) & (-20k, -20k) \\
  \hline
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \textbf{(Normal, 24/7)}
  \item \textbf{(24/7, Normal)}
\end{itemize}
...
\textbf{Interpretation:}
\begin{itemize}
  \item In equilibrium, one player stays aggressive, the other backs down
  \item Someone must “blink”
\end{itemize}
...
\textbf{Real Danger:}
\begin{itemize}
  \item If both choose 24/7 → (-20k, -20k)
  \item This is the “crash” outcome
\end{itemize}
...
\textbf{Key Insight:}
\begin{itemize}
  \item Success depends on convincing the other player to back down
  \item But if both refuse → mutual destruction
\end{itemize}
...
\textbf{The Scenario:}
\begin{itemize}
  \item Both can invest in high quality (Stag) → premium experience, more customers
  \item Or cut costs (Hare) → safe, short-term profits
  \item If both choose Stag → street becomes a destination (high payoff)
  \item If one chooses Stag while other chooses Hare → the one investing suffers
  \item Key challenge: \textbf{trust and assurance}, not competition
\end{itemize}
...
\begin{tabular}{|c|c|c|}
  ...
  \textbf{Ramesh $\backslash$ Suresh} & \textbf{Stag} & \textbf{Hare} \\
  \hline
  \textbf{Stag} & (80k, 80k) & (30k, 55k) \\
  \hline
  \textbf{Hare} & (55k, 30k) & (45k, 45k) \\
  \hline
\end{tabular}
\end{center}
...
\textbf{Key Tension:}
\begin{itemize}
  \item Stag gives highest reward—but only if both choose it
  \item Hare is safer—works even if the other doesn’t cooperate
\end{itemize}
...
\begin{itemize}
  \item If Suresh chooses Stag:
  \begin{itemize}
    \item Stag = 80k, Hare = 55k
    \item $\Rightarrow$ \underline{Stag} is better
  \end{itemize}
  
  \item If Suresh chooses Hare:
  \begin{itemize}
    \item Stag = 30k, Hare = 45k
    \item $\Rightarrow$ \underline{Hare} is better
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item If Ramesh chooses Stag → \underline{Stag} is better
  \item If Ramesh chooses Hare → \underline{Hare} is better
\end{itemize}
...
\begin{tabular}{|c|c|c|}
  ...
  & Stag & Hare \\
  \hline
  Stag & \textbf{(\underline{80k}, \underline{80k})} & (30k, 55k) \\
  \hline
  Hare & (55k, 30k) & \textbf{(\underline{45k}, \underline{45k})} \\
  \hline
\end{tabular}
\end{center}
...
\textbf{Two Nash Equilibria:}
\begin{itemize}
  \item \textbf{(Stag, Stag)} → High payoff (80k, 80k)
  \item \textbf{(Hare, Hare)} → Safer but lower payoff (45k, 45k)
\end{itemize}
...
\begin{itemize}
  \item (Stag, Stag) is better—but risky
  \begin{itemize}
    \item If one deviates → other falls to 30k
  \end{itemize}
  
  \item (Hare, Hare) is safe—but inferior
\end{itemize}
...
\textbf{Meaning:}
\begin{itemize}
  \item This is a problem of \textbf{trust and coordination}
  \item Success requires confidence that the other will also commit
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Battle of the Sexes: Bakery Partnership}

\textbf{The Scenario:}
\begin{itemize}
  \item Both want to collaborate with a bakery (Breakfast \& Coffee combo)
  \item Ramesh prefers his premium beans to be featured
  \item Suresh prefers his branded croissants to be highlighted
  \item Both want the partnership—but disagree on terms
\end{itemize}

\vspace{6pt}

\textbf{Payoff Matrix (Monthly Gains):}
...
\textbf{Key Tension:}
\begin{itemize}
  \item Coordinating gives positive payoff
  \item But each prefers a different agreement
  \item If they fail to agree → no deal (0,0)
\end{itemize}
...
\begin{itemize}
  \item If Suresh chooses Ramesh's Terms:
  \begin{itemize}
    \item Ramesh's Terms = 0, Suresh's Terms = 50k
    \item $\Rightarrow$ \underline{Suresh's Terms} is better
  \end{itemize}
  
  \item If Suresh chooses Suresh's Terms:
  \begin{itemize}
    \item Ramesh's Terms = 30k, Suresh's Terms = 0
    \item $\Rightarrow$ \underline{Ramesh's Terms} is better
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item If Ramesh chooses Ramesh's Terms → \underline{Suresh's Terms} is better
  \item If Ramesh chooses Suresh's Terms → \underline{Ramesh's Terms} is better
\end{itemize}
...
\begin{itemize}
  \item No dominant strategy exists
  \item Best action depends on coordinating with the other player
\end{itemize}
...
\begin{tabular}{|c|c|c|}
  \hline
  & Ramesh's Terms & Suresh's Terms \\
  \hline
  Ramesh's Terms & (0, 0) & \textbf{(\underline{30k}, \underline{50k})} \\
  \hline
  Suresh's Terms & \textbf{(\underline{50k}, \underline{30k})} & (0, 0) \\
  \hline
\end{tabular}
\end{center}
...
\textbf{Two Nash Equilibria:}
\begin{itemize}
  \item \textbf{(Ramesh's Terms, Suresh's Terms)} → Suresh gets more
  \item \textbf{(Suresh's Terms, Ramesh's Terms)} → Ramesh gets more
\end{itemize}
...
\begin{itemize}
  \item Both equilibria are better than (0,0)
  \item But each player prefers a different one
\end{itemize}
...
\textbf{Meaning:}
\begin{itemize}
  \item This is a \textbf{coordination problem with conflict of interest}
  \item Success depends on:
  \begin{itemize}
    \item negotiation
    \item commitment
    \item or sharing gains over time
  \end{itemize}
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Ultimatum Game: Profit Split}

\textbf{The Scenario:}
\begin{itemize}
  \item Joint catering contract worth Rs 100,000
  \item Ramesh (Proposer) suggests how to split the money
  \item Suresh (Responder) can:
  \begin{itemize}
    \item Accept → both get the proposed shares
    \item Reject → both get (0,0)
  \end{itemize}
\end{itemize}

\vspace{6pt}

\textbf{Possible Offers:}

\begin{center}
\begin{tabular}{|l|c|c|}
  \hline
  \textbf{Offer} & \textbf{If Accepted} & \textbf{If Rejected} \\
  \hline
  90K : 10K & (90k, 10k) & (0, 0) \\
  \hline
  70K : 30K & (70k, 30k) & (0, 0) \\
...
\textbf{Key Feature:}
\begin{itemize}
  \item Ramesh moves first, Suresh responds after seeing the offer
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Step 1: What Will Suresh Do?}

\textbf{Think from Suresh's perspective (Responder):}

\begin{itemize}
  \item If offered 10k:
  \begin{itemize}
    \item Accept = 10k, Reject = 0
    \item $\Rightarrow$ \underline{Accept}
  \end{itemize}
  
  \item If offered 30k:
  \begin{itemize}
    \item Accept = 30k, Reject = 0
    \item $\Rightarrow$ \underline{Accept}
  \end{itemize}
  
  \item If offered 50k:
  \begin{itemize}
    \item Accept = 50k, Reject = 0
    \item $\Rightarrow$ \underline{Accept}
  \end{itemize}
\end{itemize}
...
\textbf{Conclusion (Purely Rational):}
\begin{itemize}
  \item Suresh should accept \textbf{any positive offer}
\end{itemize}

\vspace{6pt}

\textbf{Now think as Ramesh:}
\begin{itemize}
  \item “If any offer is accepted, I should offer as little as possible”
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Equilibrium vs Reality}

\textbf{Theoretical Prediction:}

\begin{itemize}
  \item Ramesh offers the smallest possible amount (e.g., 1 rupee)
  \item Suresh accepts
\end{itemize}
...
\[
\textbf{Outcome: (Almost\ all,\ Almost\ zero)}
\]
...
\begin{itemize}
  \item People reject “unfair” offers (below 20–30\%)
  \item Common outcomes:
  \[
  \textbf{(50k,\ 50k)} \quad \text{or} \quad \textbf{(70k,\ 30k)}
  \]
\end{itemize}
...
\begin{itemize}
  \item Humans care about \textbf{fairness}, not just money
  \item Rejecting unfair offers is a way to punish unfair behavior
\end{itemize}
...
\begin{itemize}
  \item To succeed, leave enough value for others
  \item Purely “rational” strategies may fail in real life
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Public Goods: Street Lighting Problem}

\textbf{The Scenario:}
\begin{itemize}
  \item Street lighting costs Rs 20,000 total
  \item Each can contribute Rs 10,000 or nothing
  \item If installed, both benefit (Rs 15,000 each)
\end{itemize}
...
\begin{itemize}
  \item If both contribute:
  \begin{itemize}
    \item Pay 10k, get 15k → net 5k each
  \end{itemize}
  
  \item If only one contributes:
  \begin{itemize}
    \item Contributor: pays 10k, gets 0 benefit alone → -10k
    \item Free-rider: pays 0, gets 15k → +15k
  \end{itemize}
  
  \item If no one contributes:
  \begin{itemize}
    \item No lighting → (0,0)
  \end{itemize}
\end{itemize}
...
\textbf{Key Tension:}
\begin{itemize}
  \item Everyone benefits from the public good
  \item But each prefers that the other pays
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Finding Dominant Strategy}

\textbf{For Ramesh:}

\begin{itemize}
  \item If Suresh contributes:
  \begin{itemize}
    \item Contribute = 5k, Free-ride = 15k
    \item $\Rightarrow$ \underline{Free-Ride} is better
  \end{itemize}
  
  \item If Suresh free-rides:
  \begin{itemize}
    \item Contribute = -10k, Free-ride = 0
    \item $\Rightarrow$ \underline{Free-Ride} is better
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Whether Ramesh contributes or not, Suresh is better off choosing \underline{Free-Ride}
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item \underline{Free-Ride} is the dominant strategy for both players
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Nash Equilibrium and Public Goods Problem}

\textbf{Since both choose dominant strategy \underline{Free-Ride}, the Nash equilibrium is:}

\[
\textbf{(Free\text{-}Ride,\ Free\text{-}Ride)} = (0, 0)
\]
...
\begin{itemize}
  \item Best collective outcome: (5k, 5k) when both contribute
  \item But rational choices lead to (0,0)
\end{itemize}
...
\textbf{Key Insight:}
\begin{itemize}
  \item Individuals avoid paying, hoping others will
  \item Result: public good is not provided
\end{itemize}

\vspace{6pt}

\textbf{Real-World Solutions:}
\begin{itemize}
  \item Taxes / mandatory contributions
  \item Social norms and reputation
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Matching Pennies: Timing Game Intuition}

\textbf{The Scenario:}
\begin{itemize}
  \item Ramesh and Suresh compete for morning customers
  \item Each chooses: Open Early (6 AM) or Normal (8 AM)
  \item If choices match → Ramesh wins
  \item If choices differ → Suresh wins
\end{itemize}
...
\begin{tabular}{|c|c|c|}
  \hline
  \textbf{Ramesh $\backslash$ Suresh} & Early & Normal \\
  \hline
  Early & (1, -1) & (-1, 1) \\
  \hline
  Normal & (-1, 1) & (1, -1) \\
...
\textbf{Key Insight:}
\begin{itemize}
  \item One player's gain is exactly the other's loss (zero-sum)
  \item No possibility of cooperation or mutual benefit
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Best Responses (Why No Stable Choice)}

\textbf{For Ramesh:}

\begin{itemize}
  \item If Suresh chooses Early:
  \begin{itemize}
    \item Early = 1, Normal = -1
    \item $\Rightarrow$ \underline{Early} is better
  \end{itemize}
...
\begin{itemize}
  \item If Ramesh chooses Early → \underline{Normal} is better
  \item If Ramesh chooses Normal → \underline{Early} is better
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item No dominant strategy for either player
  \item Each player wants to \textbf{mismatch} the other
\end{itemize}

\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{No Nash Equilibrium in Pure Strategies}

\textbf{Check each outcome:}

\begin{itemize}
  \item (Early, Early): Suresh can switch → improves from -1 to 1
  \item (Early, Normal): Ramesh can switch → improves from -1 to 1
  \item (Normal, Early): Ramesh can switch → improves from -1 to 1
  \item (Normal, Normal): Suresh can switch → improves from -1 to 1
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
  \item No cell where both are best responding
  \item \textbf{No pure Nash equilibrium exists}
\end{itemize}

\vspace{8pt}

\textbf{Solution: Mixed Strategy}

\begin{itemize}
  \item Each player randomizes:
  \begin{itemize}
    \item Play Early with 50\%
    \item Play Normal with 50\%
  \end{itemize}
\end{itemize}
...
\textbf{Key Insight:}
\begin{itemize}
  \item Being predictable makes you exploitable
  \item Optimal strategy is to be \textbf{unpredictable}
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Advanced Concepts}
\begin{center}
...
\begin{columns}
  ...
  \end{columns}
  \vspace{4pt}
  \begin{alertblock}{Plain English}
    A plan is subgame-perfect only if you would actually follow through at every
    branch --- including ones you hope never to reach.
    A threat you would abandon under pressure is not a plan; it is a bluff.
  \end{alertblock}
\end{frame}
    ...
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item Update beliefs using Bayes' Rule as evidence arrives (posterior $\propto$ prior $\times$ likelihood of observed signal)
...
\section{Credibility and Commitment}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Credibility and Commitment}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Strategic Interaction}
\begin{center}
{\Large Strategic Interaction}
\end{center}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Competitive Advantage}
\begin{center}
...
\begin{columns}
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
        ...
        \item \textbf{Situation}: Dominant firms in markets with moderate entry barriers, commodity markets with cost-advantage incumbents, digital platforms with significant scale economies.
      \end{itemize}
    \end{column}
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Algorithmic Strategy: When Algorithms Play the Game}
\begin{columns}
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
        \item \textbf{Introduction}: When competing firms deploy pricing or bidding
          algorithms that observe and react to each other, Nash equilibria
          emerge \textit{without} human intent or explicit coordination.
          The result can resemble collusion but arises from independent
          reactive machines.
        \item \textbf{Situation}: E-commerce repricing, airline yield management,
          digital ad auctions, ride-hailing surge pricing,
    ...
    \begin{column}[T]{0.5\linewidth}
      \begin{itemize}
        \item \textbf{Steps}:
        \begin{itemize}
          \item Identify whether competitors use reactive pricing systems
          \item Audit your own algorithm's update frequency and reaction function
          \item Anticipate that algorithms will converge to a Nash equilibrium,
            possibly a price-war or supra-competitive outcome
          \item Design algorithms with long-run profit as the objective,
            not just short-run market share
          \item Monitor for algorithmic tacit collusion --- regulators are catching up
        \end{itemize}
        \item \textbf{Examples}: Amazon third-party seller bots converging on
          near-identical prices; airline revenue management systems; Google Ads
          second-price auctions; Uber/Ola surge pricing under simultaneous peak demand.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Information}
\begin{center}
{\Large Information}
\end{center}
\end{frame}
...
\section{Negotiation and Bargaining}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Negotiation and Bargaining}
\begin{center}
...
\section{Market Design and Mechanisms}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Market Design and Mechanisms}
\begin{center}
...
\section{Collective Challenges}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Collective Challenges}
\begin{center}
{\Large Collective Challenges}
\end{center}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Prospect Theory}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Applying Game Theory: This Week \& This Month}
  \begin{columns}
    \begin{column}[T]{0.48\linewidth}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]\frametitle{Applying Game Theory: Ongoing Practice}
  \begin{columns}
    \begin{column}[T]{0.48\linewidth}
...
\begin{block}{Thinking Strategically \ldots}
  \begin{itemize}
    ...
    \item ``In the real world, the game isn't broken. The players are human.'' --- Author's paraphrase of Kahneman \& Tversky
